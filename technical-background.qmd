...
- `{package}` will be used to denote a specific package, e.g., `{DataFrames.jl}` denotes the `{DataFrames.jl}` package.
- `package.function()` will be used to denote a function that is part of a specific package, e.g., `DataFrames.subset()` denotes the `subset()` function from the `{DataFrames.jl}` package.
...
- _**files**_ will be used to highlight file names, e.g., _**README.md**_ or _**LICENSE**_.
- _italics_ will be used for emphasis in certain circumstances, e.g., signifying a question from an interactive terminal command.
...
But, in the interest of consistency and ease of collaboration, I would strongly recommend you use the [`{JuliaFormatter.jl}` package](https://domluna.github.io/JuliaFormatter.jl/dev/) to format your code according to a specific style guide.
In this book, I will use a lightly modified version of the [Blue style guide](https://github.com/invenia/BlueStyle), and you can see the specific changes to the default style in the _**.JuliaFormatter.toml**_ file in the root directory of this book's GitHub repository.
In the next section, I will discuss how you can use the `{JuliaFormatter.jl}` package to format your code.
