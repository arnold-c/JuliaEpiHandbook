...
url = base_url * "data_source=" * data_source * "&signal=" * signal * "&time_type=" * time_type * "&geo_type=" * geo * "&time_values=" * time_values * "&geo_values=" * geo_values
```
...
Unfortunately the `body` property is a `Vector{UInt8}` and we can't use it in our code, so we need to convert it to a `String` first before we can manipulate it.
...
parsed = Serde.parse_json(String(copy(response.body)))
...
for i in eachindex(epidata)
    vals[i] = epidata[i]["value"]
    dates[i] = Date("$(epidata[i]["time_value"])", dateformat"yyyymmdd")
end
```
